...
@section('title', 'Pedido Rechazado por Gerencia')

@section('content')
    <p style="font-size: 18px; margin-bottom: 20px;">Hola, <strong>{{ $pedido->creador->nombre_completo ?? 'Usuario' }}</strong>.</p>
    
    <p style="margin-bottom: 25px;">
        Te informamos que tu pedido con consecutivo <strong>#{{ $pedido->consecutivo }}</strong> ha sido revisado por la <strong>Gerencia</strong> y <strong style="color: #be123c;">no ha sido aprobado</strong>.
    </p>

    <!-- Card de Motivo -->
    <div style="background-color: #fff1f2; border-left: 4px solid #f43f5e; padding: 20px; border-radius: 4px; margin-bottom: 30px;">
        <h3 style="margin-top: 0; margin-bottom: 10px; font-size: 16px; color: #9f1239;">Motivo del Rechazo de Gerencia:</h3>
        <p style="margin: 0; color: #881337; line-height: 1.5;">{{ $motivo ?? $pedido->motivo_rechazado_gerencia ?? 'No especificado' }}</p>
    </div>
    ...
    <p style="margin-top: 30px; font-size: 14px; color: #64748b;">
        Por favor, atiende las observaciones de la gerencia.
    </p>

    <div style="text-align: center; margin-top: 30px;">
        <a href="{{ env('FRONTEND_URL', 'http://localhost:5173') }}/cp-pedidos/{{ $pedido->id }}" class="button" style="background: #ef4444 !important; box-shadow: 0 4px 6px rgba(239, 68, 68, 0.2) !important;">Ver Pedido</a>
    </div>
@endsection
